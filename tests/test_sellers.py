...
async def test_create_seller_duplicate_email(async_client, db_session):
    ...
    seller = Seller(
        first_name="First", last_name="User", e_mail=e_mail, password="password123"
    )
...
async def test_update_seller(async_client, db_session):
    ...
    seller = Seller(
        first_name="Original", last_name="Name", e_mail=e_mail, password="password123"
    )
    db_session.add(seller)
    await db_session.commit()
    await db_session.refresh(seller)
...
async def test_delete_seller(async_client, db_session):
    ...
    seller = Seller(
        first_name="To", last_name="Delete", e_mail=e_mail, password="password123"
    )
    db_session.add(seller)
    await db_session.commit()
    await db_session.refresh(seller)
